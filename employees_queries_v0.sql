...
SELECT d.dept_name, em.gender, em.emp_no, em.from_year, em.to_year, cy.calender_year,
	CASE 
		WHEN cy.calender_year BETWEEN from_year AND to_year THEN 1
        ELSE 0 
	END AS active_man
FROM emp_manager AS em
JOIN t_departments AS d
ON d.dept_no = em.dept_no
CROSS JOIN calender_year AS cy
ORDER BY em.emp_no, calender_year
)
SELECT may.dept_name, may.gender, may.calender_year, COUNT(may.active_man) AS num_of_managers
FROM manager_active_year AS may
WHERE may.active_man = 1
